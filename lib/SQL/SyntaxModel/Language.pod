...
this document goes into more detail on them):
	...
	- all bind variables are named (rather than positional)
...
root, and it must not be set when the Node is not a root.  Note that all
attributes having an attribute type of "N.CCE" will also have an implicit
dependant conditionally mandatory status.
...
Attribute List:
	...
	command_arg_1 - N.CCE(command_type) - an argument to the above command
	command_arg_2 - N.CCE(command_type) - an argument to the above command

	Note: See the 'command_type' enumerated list documentation to see 
	what Node type it expects for its command arguments.  Note that one 
	or both of the arguments may be optional or required based on what 
	the command_type is; most command types take exactly 1 argument.
