...
function (a grouping func, like 'count')
	...
	NVL     - aka COALESCE (SQL-92 name) - returns arg 1 if it is not null, else arg 2
...
These commands must be called in a "transaction" context:
	...
	SEQU_LIST    - fetches a list of sequences in schema of open db instance
	SEQU_INFO    - fetches some detailed info about a sequence
	SEQU_VERIFY  - compares a provided sequence spec to an actual table
	SEQU_CREATE  - creates a new sequence in schema of open db inst
	SEQU_DELETE  - deletes an existing sequence in open db inst (if it can)
	SEQU_CLONE   - creates a clone of an existing sequence with different name
	SEQU_UPDATE  - alters schema for an existing sequence
...
be 5 digits wide with 2 digits on the right of the decimal point); 3. The
...
Attribute List:
	...
	num_scale - L.uint - for non-integral numbers, how many digits can be on the right of the decimal point
