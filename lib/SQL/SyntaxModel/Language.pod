...
This is a brief list of major non-supported features (at present):
	...
	- table hierarchies or subtables; anything not a "base table"
	- object data types
...
SQL::SyntaxModel v0.18 domains are under 'blueprints'.  We describe most
structures in 'blueprints', including database tables (with constraints),
arbitrarily complex database views, and various kinds of routines (such as
stored procedures, user defined functions, and basic DML like selects, inserts,
updates, deletes), both those stored in a database schema or in an application.
 We can represent multiple database catalogs and applications at once, whether
or not they talk to each other; how they inter-relate is also represented.
...
routines (even if only their child INSIDE have them), either database or
...
These commands must be called in a "transaction" context:
	...
	SCHEMA_INFO   - fetches some detailed info about a schemas
	SCHEMA_VERIFY - compares a provided schema spec to an actual schemas
	SCHEMA_CREATE - creates a new schema in catalog of open db inst
	SCHEMA_DELETE - deletes an existing schema in open db inst (if it can)
	SCHEMA_CLONE  - creates a clone of an existing schema with different name
	SCHEMA_UPDATE - alters details for an existing schema
	DOMAIN_LIST   - fetches a list of domains of open db instance
	DOMAIN_INFO   - fetches some detailed info about a domain
	DOMAIN_VERIFY - compares a provided domain spec to an actual domain
	DOMAIN_CREATE - creates a new domain in schema of open db inst
	DOMAIN_DELETE - deletes an existing domain in open db inst (if it can)
	DOMAIN_CLONE  - creates a clone of an existing domain with different name
	DOMAIN_UPDATE - alters details for an existing domain
	SEQU_LIST    - fetches a list of sequence generators in schema of open db instance
	SEQU_INFO    - fetches some detailed info about a sequence
	SEQU_VERIFY  - compares a provided sequence generator spec to an actual table
	SEQU_CREATE  - creates a new sequence generator in schema of open db inst
	SEQU_DELETE  - deletes an existing sequence generator in open db inst (if it can)
	SEQU_CLONE   - creates a clone of an existing sequence with different name
	SEQU_UPDATE  - alters details for an existing sequence generator
	TABLE_LIST   - fetches a list of tables in schema of open db instance
	TABLE_INFO   - fetches some detailed info about a table
	...
	TABLE_UPDATE - alters details for an existing table
	VIEW_LIST    - fetches a list of views in schema of open db instance
	VIEW_INFO    - fetches some detailed info about a view
	...
	VIEW_UPDATE  - alters details for an existing view
	ROUTINE_LIST   - fetches a list of routines/procs/funcs of open db instance
	ROUTINE_INFO   - fetches some detailed info about a routine
	...
	ROUTINE_UPDATE - alters details for an existing routine
...
the Node type has no candidate attributes).  The flag says "PP".
...
attribute must always be populated (not be null); in some cases, it means you
explicitly provide a 'default' value such as zero.  If the flag says
"MC[EE|R](...)" then the attribute is only sometimes mandatory.  A flag of
"MCEE(foo=bar[|baz...])" means that the attribute is mandatory only when
another, same-Node enumerated attribute named in the parenthesis (left side of
'=') equals one of the values given in the parenthesis (right side of '=').  A
flag of "MCR(foo)" only applies to Node types that can have parent Nodes of the
same type, arranged in a tree; the attribute named in the parenthesis is null
if the current Node is a root of its tree (could be of one element), and it is
set when it is not a root; an MCR attribute must be set when the Node is a
root, and it must not be set when the Node is not a root.

Note that for for any Node Type that has an attribute of 'name', that attribute
is often one and the same thing as the 'identifier' in SQL; eg, a table 'name'
is the table's 'identifier'.
...
Attribute List:
	...
	schema - N.schema - PP - MCEE(view_context=SCHEMA) - the schema that this view is in, if any
	name - L.cstr - MA - the name this view is identified by, if any
	application - PP - N.application - MCEE(view_context=APPLIC) - if view in app space, not app routine, an app is our parent
	routine - PP - N.routine - MCEE(view_context=ROUTINE) - parent view of this one, if any; empty if this is the root view
	p_view - PP - N.view - MCEE(view_context=INSIDE) - parent view of this one, if any; empty if this is the root view
	match_all_cols - L.bool - use with view_type=MATCH; if true, this view matches every col; if false, view_col has list
	c_merge_type - E.compound_operator - MCEE(view_type=COMPOUND) - how to merge any child rowsets/views of this one together, if any
...
Attribute List:
	...
	domain - N.domain - MCEE(expr_type=CAST) - cast single child expression into this data type / domain
...
Attribute List:
	...
	trigger_per_row - L.bool - MCEE(routine_type=TRIGGER) - true, exec each row, false, each stmt
...
Attribute List:
	...
	view_for_dml - N.view - if the statement is an insert/update/delete/etc (non-cursor) defined by a view, name it here
