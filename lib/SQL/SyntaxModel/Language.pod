...
Attribute List:
	...
	id - NODE_ID - MA - unique identifier for each Node within this Node Type
	priv_on - N.privilege_on - MA - what "privilege_on" Node this option is associated with
...
Attribute List:
	...
	id - NODE_ID - MA - unique identifier for each Node within this Node Type
	schema - N.schema - MA - the schema that this domain is in
	...
	calendar - E.calendar - MC(base_type=DATM_FULL,DATM_DATE) - what calendar the date and time (or date only) is in
...
Attribute List:
	...
	view_col - N.view_col - MC(view_part=RESULT|INTO) - the view interface/result column we are defining the value of
	set_view_col - N.view_src_col - MC(view_part=SET) - this is the column we are setting the value of in insert or update op
	view_src_arg - N.view_src_arg - MC(view_part=FROM) - arg of named subquery used in "from" we are passing value to
	...
	domain - N.domain - MC(expr_type=CAST) - cast single child expression into this data type / domain
	sequence - N.sequence - MC(expr_type=SEQN) - the sequence generator we will get the "next" value for
...
Attribute List:
	...
	table - N.table - if this routine is a trigger on a table, then this is the table to watch
	view - N.view - if this routine is a trigger on a named view, then this is the view to watch
	...
	trigger_event - E.basic_trigger_event - MC(routine_type=TRIGGER) - when a trigger executes
	trigger_per_row - L.bool - MC(routine_type=TRIGGER) - true, exec each row, false, each stmt
...
Attribute List:
	...
	domain - N.domain - MC(var_type=SCALAR) - user customized data type of scalar arg
	curs_view - N.view - MC(var_type=CURSOR) - put reference to the view defining this cursor here
...
Attribute List:
	...
	domain - N.domain - MC(var_type=SCALAR) - user customized data type of scalar variable
	...
	curs_view - N.view - MC(var_type=CURSOR) - put the view defining cursor here
