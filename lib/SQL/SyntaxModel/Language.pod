...
B<Example List>.  I<TODO: Replace the "Unique Constraint List" sections.>
...
attribute is implicitly always-unique within Nodes of the same Node type.
...
The "Attribute List" also contains a few details that pertain to deferred
constraints.  One deferred constraint is that a Node's NODE_ID attribute must
be set; that is, this constraint is only deferred for Nodes that are not in
Containers.  Another deferred constraint is that exactly one primary-parent
attribute must be set, if the Node has any "PP" attributes.  Another deferred
constraint is that any "MA" attributes are populated.  All "Attribute List" 
deferred constraints can be tested on Nodes of any status, including "Alone".
...
List" constraints are deferred and can be tested on Nodes of any status,
including "Alone".
...
deferred and can be tested on Nodes of any status, including "Alone".
...
Local Attribute Dependencies List:
	...
	view_part:
		view_col - RESULT,INTO - MA
		set_routine_arg,set_routine_var - INTO - MA
		set_view_col - SET - MA
...
Local Attribute Dependencies List:
	...
	routine_type:
		return_var_type - ANONYMOUS,FUNCTION
		trigger_event - TRIGGER - MA
		trigger_per_stmt - TRIGGER - MA
	return_var_type:
		return_domain - SCALAR - MA

	TODO: Make return_var_type mandatory for FUNCTION only.
...
Attribute List:
	...
	call_uproc - N.routine - this is the user-defined stored function (routine) to call
...
Exclusive Attribute Groups List:
	...
	type - is_file_based,is_local_proc,is_network_svc - MA
