...
These commands must be called in a "transaction" context:
	...
	SEQU_UPDATE  - alters schema for an existing sequence generator
...
Attribute List:
	...
	Note: The name of a trigger (as required by SQL-2003) is defined in the 
	routine Node that implements the trigger body.

	Note: SQL-2003 seems to suggest triggers can attach to tables or named views.
...
Attribute List:
	...
	name - L.cstr - MC(view_context=SCHEMA|view_type=RECURSIVE) - the name this view is identified by, if any
...
Attribute List:
	...
	name - L.cstr - MA - name routine ident by in a [schema, p_routine, app], or name of trigger it impl
